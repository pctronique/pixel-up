class EnumTypeMort {
  static NULL = -1;
  static TOMBER = 0; // Vous êtes tombé.
  static REQUIN = 1; // Vous avez été mangé par un requin.
  static AVION = 2; // Vous avez été percuté par un avion.
  static ASTEROIDE = 3; // Vous avez été percuté par un astéroïde.
  static FEU = 4; //Vous avez été brûlé par le feu.
  static HELICOPTERE = 5; //Vous avez été percuté par un hélicoptère.
  static LAVE = 6; // Vous avez été brûlé par la lave.
  static MEDUSE = 7; // Vous avez été piqué par une méduse.
  static METEORITEFEU = 8; // Vous avez été brûlé par une météorite en feu.
  static POISSON = 9; // Vous avez été percuté par un poisson.
  static SATELLITE = 10; // Vous avez été percuté par un satellite.
  static NACELLE = 11; // Vous avez percuté une plateforme.
  static NUAGE = 12; // Vous avez été percuté par un nuage.
  static OISEAU = 13; // Vous avez été percuté par un oiseau.
  static PIERRE = 14; // Vous avez percuté une pierre.
  static VAISSEAU = 15; // Vous avez été percuté par un vaisseau.
  static VAISSEAUJAUNE = 16; //Vous avez été percuté par un vaisseau.
  static RADFISH = 17; // Vous avez été tué par un poisson radioactif.
  static NUAGEGRIS = 18; // Vous avez été tué par un nuage orageux.
  static NACELLECASSEE = 19; // Vous êtes tombé dans le vide.
  static TENUE_NORMAL = 20;
  static TENUE_MER = 21;
  static TENUE_TERRE = 22;
  static TENUE_CIEL = 23;
  static TENUE_ESPACE = 24;
}
